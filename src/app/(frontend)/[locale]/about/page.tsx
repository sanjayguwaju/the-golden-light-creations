...
export default async function AboutPage() {
  ...
  return (
    <div className="bg-[#0A0A0A] text-white min-h-screen pt-28 pb-20">
      ...
      <section className="py-20 sm:py-24 px-4 sm:px-8 max-w-7xl mx-auto border-b border-white/5">
        <div className="text-center max-w-2xl mx-auto mb-12 sm:mb-16">
          <span className="font-montserrat text-xs font-bold tracking-[0.4em] text-[#F5B301] uppercase block mb-3">
            How We Work
          </span>
          <h2 className="font-bebas text-3xl sm:text-6xl text-white uppercase leading-none">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
          {values.map((v, idx) => (
            <div
              key={idx}
              className="bg-white/[0.02] border border-white/[0.08] hover:border-[#F5B301]/40 p-6 sm:p-8 transition-all duration-300 hover:-translate-y-1.5"
            >
              <div className="w-12 h-12 border border-[#F5B301]/30 flex items-center justify-center mb-6">
                {v.icon}
              </div>
              <h3 className="font-montserrat text-base font-bold uppercase tracking-wider text-white mb-2 sm:mb-3">
                {v.title}
              </h3>
